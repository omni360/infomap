...
function init_webgl() {
	...
	var globe = new THREE.Mesh(
		...
		new THREE.MeshBasicMaterial({'color': new THREE.Color(0x050E16), opacity: .1, blending: THREE.AdditiveBlending})
	);
	...
	var light = new THREE.PointLight(0xc0c0c0, 3);
	light.position.z += 5000;
	scene.add(light);
	
	createShapeMap(earth_data);
...
function startRibbonAnimation (obj, delay) {
	...
	var timeTween = new TWEEN.Tween(o)
		...
		.onUpdate(function (){
			...
			var screen_coord = worldToScreen( lineContainer.localToWorld(o.endPoint.clone().multiplyScalar((radius + lineHeight) / radius)), projector, camera);
			
			dstPos = {
				x: screen_coord.x - w / 2,
				y: screen_coord.y - h / 2
			};
...
function createDot(para, color) {
	...
	textMesh.scale.set(spriteScale / 7, spriteScale / 7, spriteScale / 7)
	return textMesh;
...
function createTip(para, color) {
	...
	textMesh.material.opacity = 1;
	textMesh.scale.set(spriteScale * 1.7, spriteScale * 1.7, spriteScale * 1.7);
	return textMesh;
...
function startTimer(){
	...
	var timer = setInterval(function (){
		...
		consoleTrace('放马源目标： IP<span class="infored">'+srcInfo['ip']+'</span>, 地理位置<span class="infored">'+srcInfo['area']['cities']+
			'</span>	被感染目标： IP<span class="infored">'+srcInfo['ip']+'</span>, 地理位置<span class="infored">'+dstInfo['area']['cities']+'</span>');
...
function createShapeMap(areas){
	...
	for (var i = 0; i < areas.length; i++){
		...
		for (var j = 0; j < areas[i].length; j++){
			...
			if (point.name){
				...
				shape_pts.push(new THREE.Vector2(point.x, point.y));
		...
		if (shape_pts.length > 0){
			...
			//geometry.applyMatrix( new THREE.Matrix4().makeTranslation( 0, 0, 16) );
			
			var faces = geometry.faces;
			
			var color = new THREE.Color(0x186E96);
	...
	vertices = mergeGeometry.vertices;
	for (var k = 0; k < vertices.length; k++){
		
		var sphere_v = coordToSphereVector(vertices[k].x, vertices[k].y, 100);
		//vector = new THREE.Vector3(sphere_v.x, -sphere_v.z, sphere_v.y);
		
		mergeGeometry.vertices[k] = sphere_v;
	}

	shapeMap = new THREE.Mesh( 
		mergeGeometry, 
...
function coordToSphereVector(x, y, r){
	...
	var phi = (180 * mapWidth / 360 - y) * Math.PI / (180  * mapWidth / 360);
    var theta = (180 * mapHeight / 180 - x) * Math.PI / (180 * mapHeight / 180);
...
function render() {
	...
	if (dstPos){
		...
		camera.position.x += (-400 * Math.sin(dstPos.x / 1000) - camera.position.x) / 30;
		camera.position.y += (-400 * Math.sin(dstPos.y / 1000) - camera.position.y) / 30;
