	...
	$.JMSlideshow.prototype 	= {
		...
		_loadEvents			: function() {
			...
			if( this.$dots ) {
				...
				$('.detail-tab-index .statistics-card-label').each(function(i){
					$(this).css({
						'left': 103*i + 20
					}).attr({'data': i});
					$(this).click(function(){
						activeTabC = $('.tab-active');
						activeTabC.removeClass('tab-active');
						preActiveIndex = activeTabC.attr('data');
						currentPage = activeTabC.attr('data');
						$(this).addClass('tab-active');
						activeTabC = $('.tab-active');
						detailPanel.TablePageBtnUpdate(i);
