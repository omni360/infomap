...
OOPTEST.System.prototype.initSpace3D = function (complateFun){
	...
	//创建地板
	var floor = new Space3D.Floor({color: 0x19222C});
	scene.add(floor);
	...
	//创建拓扑主场景
	this.topo = Topo = new Space3D.TopoSystem({'viewport': mainViewport, 'dataModel': dataModel});
	scene.add(this.topo);
	
	this.addEvent();
}

OOPTEST.System.prototype.update = function (xml){
	...
	gateway.post(OOPTEST.Gateways['getPreviewTopoJson'], function (topo_info){
		...
		Topo.update({'data': topo_info});
		
	});
	
}

OOPTEST.System.prototype.go_left = function() {
	
    this.mainViewport.position.x -= 10, 
    this.mainViewport.target.position.x -= 10, 
    this.middlePoint.position = this.mainViewport.target.position;
    
}

OOPTEST.System.prototype.go_right = function() {
...
OOPTEST.System.prototype.go_front = function() {
   
    this.mainViewport.position.z -= 10, 
    this.mainViewport.target.position.z -= 10, 
    this.middlePoint.position = this.mainViewport.target.position;
    
}

OOPTEST.System.prototype.go_back = function() {
