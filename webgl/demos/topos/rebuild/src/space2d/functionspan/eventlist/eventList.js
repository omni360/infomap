...
Space2D.eventList.prototype.createHtmlDom = function(){
	...
	this.gateway.request(OOPTEST.Gateways['geteventListPage'], function (page){
		// _temp.append(page);
		$('#list').append(page);
		$("#eventtable").css({'height': (window.innerHeight-142)});
        ...
        _this.getList();
        $('#eventlist footer button').click(function(){
        	_this.getList();
        })
	});
}
...
Space2D.eventList.prototype.getList = function(){
	...
	this.getway.request(OOPTEST.Gateways['geteventList'], function (res){
		...
		//var list = res.list;
		var tableHtml = '',_iptr = '';
		console.info(history_event_list);
		for(var i = 0; i<res.length ;i++){
			if( i%2 == 0 ){
				_iptr = "<tr><input type='hidden' value='"+res[i]['topological_id']+"'/><td class='td_serial'>" + res[i]['detect_id'] + "</td><td class='td_ip'>";
		...
		$('#eventtable table tr').dblclick(function(){
        	...
        	console.info(_did+','+_dtype+','+_tpid);
        });
        $("#eventlist footer span").html('共 ' + res.length + '条');
	});
