...
Space2D.historyMsgEvent = function (options){
 ...
 * Init 开始执行获取数据存入数组
 */
Space2D.historyMsgEvent.prototype.initialize = function (){
 ...
 * 开始显示Msg
 */
Space2D.historyMsgEvent.prototype.play = function (){
    ...
    if(Space2D.Globle.timerLogFlag && ((new Date()).getTime()-this.Milliseconds) >= 1500 || this.animateCounter >= 100){
    	...
    	if(this.cardNum < 4){
    		this.modelObj.cardSet  && this.modelObj.cardSet.animate({transform: 't0,' + (this.cardNum*60-240)}, 300); 
 ...
 * 设置播放标识
 */
Space2D.historyMsgEvent.prototype.SetPlayFlag = function (_value){
