...
Space3D.TopoSystem.prototype.animateNodes = function (isResetScene){
	...
	if (isResetScene){
		var tween = new TWEEN.Tween({radius: _this.sys.cameraRadius, phi: 180, opacity: 0})
			.to({radius: 3500, phi: 60, theta: -30, opacity: 1}, (this.data['level'].length - 1) * 1000)
	    	.easing(TWEEN.Easing.Linear.None)
	    	.delay(3000)
	    	.onUpdate(function (){
	    		_this.sys.cameraRadius = this.radius;
	    		_this.sys.phi = this.phi;
	    		_this.sys.theta = this.theta;
	    		//_this.sys.lastPhi = this.phi;
	    		//_this.sys.lastTheta = this.theta;
