...
Space3D.TrojanEvent = function(nodeIdDict, dataModel){
...
Space3D.TrojanEvent.prototype = Object.create(Space3D.SecurEvent.prototype); 


Space3D.TrojanEvent.prototype.initialize = function (){
    
}

//重新组织后台数据
Space3D.TrojanEvent.prototype.setData = function(result) {
        ...
        if ( this.eventNodeList.length == 0 ){
	        ...
	        for ( var k = 0; k < result.length; k++ ){
	        	...
	        	if ( k < result.length - 1 ){
		        	...
		        	this.tempGroup.push(result[k + 1]);
        	...
        	for ( var k = 0; k < result.length; k++ ){
	        	...
	        	if ( k < result.length - 1 ){
		        	...
		        	this.tempGroup.push(this.eventNodeList[this.eventNodeList.length - 1]);
		        	this.tempGroup.push(result[k]);
		        	this.eventNodeList.push(result[k]);
		        	this.eventGroups.push(this.tempGroup.concat());
		        	this.tempGroup = [];
...
Space3D.TrojanEvent.prototype.createModels = function (){
        ...
        for (item_index = start_index; item_index < this.eventGroups.length; item_index++){
            if ( this.eventGroups[item_index].length > 1 ){
                ...
                for (i = 0; i < itemGroup.length-1; i++){
                    ...
                    //var mark = new Space3D.Mark({map: Space3D.TexturesLibrary['Folder'], position: startPosition});
                    var mark = new Space3D.Cone({size: 300});
                    mark.position = startPosition;
                    mark.setDirection(endPosition.clone().subSelf( startPosition).normalize());
                    
                    //Space3D.TexturesLibrary['TheftSource']
                    //Space3D.TexturesLibrary['TheftTarget']
                    
                    var startmark = startPosition.ftp_node_class === "ftp-server" ? new Space3D.Mark({ position: startPosition, color: 0xFF0000, map: Space3D.TexturesLibrary['FtpServer']}) : (
                    	startPosition.ftp_node_class === "ftp-client" ? new Space3D.Mark({ position: startPosition, color: 0xFF0000, map: Space3D.TexturesLibrary['FtpClient']}) : null
                    );
                    
                    var endmark = endPosition.ftp_node_class === "ftp-server" ? new Space3D.Mark({ position: endPosition, color: 0xFF0000, map: Space3D.TexturesLibrary['FtpServer']}) : (
                    	endPosition.ftp_node_class === "ftp-client" ? new Space3D.Mark({ position: endPosition, color: 0xFF0000, map: Space3D.TexturesLibrary['FtpClient']}) : null
                    );
...
Space3D.TrojanEvent.prototype.start = function (){
    ...
    if (!this.playSwitch){
    	...
    	for (var i = 0; i < this.models.length; i++){
    		...
    		if (i == index || i == this.models.length - 1){
    			...
    			this.models[i].mark.cone.visible = false;
...
Space3D.TrojanEvent.prototype.play = function (){
...
Space3D.TrojanEvent.prototype.pause = function (){
	
    this.playSwitch && (this.playSwitch = false);
    
}

Space3D.TrojanEvent.prototype.stop = function (){
    ...
    for (i = 0; i < this.models.length; i++){
        ...
        this.models[i].line.visible = this.models[i].mark.cone.visible = false;
...
Space3D.TrojanEvent.prototype.goTo = function (percent){
	...
	if ( this.dataModel == "history" ){
		...
		for (var i = 0; i < show_models.length; i++){
			...
			if (i == show_models.length - 1){
				...
				show_models[i].line.visible = show_models[i].mark.cone.visible = true;
				...
				show_models[i].mark.cone.visible = false;
		...
		for (var i = 0; i < hide_models.length; i++){
			...
			hide_models[i].line.visible = hide_models[i].mark.cone.visible = false;
        ...
        obj.line.visible = obj.mark.cone.visible = true;
        if (obj.percent <= 1){
            switch(obj.percent){
                case 0:
