...
Space3D.FtpEvent = function(nodeIdDict, dataModel){
...
Space3D.FtpEvent.prototype = Object.create(Space3D.SecurEvent.prototype); 


Space3D.FtpEvent.prototype.initialize = function (){
    
}

//重新组织后台数据
Space3D.FtpEvent.prototype.setData = function(result) {
    ...
    if (this.eventNodeList.length == 0 || ( result.length > 1 && this.eventNodeList.length > 0)) {
        for (k in result){
            if (this.eventNodeList.length == 0 || k != 0){
                ...
                this.nodeIdDict[result[k].node_id]['ftp_node_class'] = result[k]['node_class'],
                this.nodeIdDict[result[k].node_id]['ftp_node_status'] = result[k]['status']
                !this.nodeIdDict[result[k].node_id]['ftp_img'] && (this.nodeIdDict[result[k].node_id]['ftp_img'] = false);
                
                if (this.tempGroup.length == 0){
                    this.prevNodeClass = result[k]['node_class'];
                }
                this.tempGroup.push(result[k]);
                if ( (this.prevNodeClass == "ftp-server" && result[k]['node_class'] == 'ftp-client') ||
                     (this.prevNodeClass == "ftp-client" && result[k]['node_class'] == 'ftp-server')){
...
Space3D.FtpEvent.prototype.createModels = function (){
...
/*
Space3D.FtpEvent.prototype.start = function (){
...
Space3D.FtpEvent.prototype.play = function (){
...
Space3D.FtpEvent.prototype.pause = function (){
...
Space3D.FtpEvent.prototype.stop = function (){
...
Space3D.FtpEvent.prototype.goTo = function (timestamp){
