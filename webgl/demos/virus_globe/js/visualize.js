...
function getVisualizedMesh( linearData, year, countries, exportCategories, importCategories ){
	...
	for( i in bin ){
		...
		if( (useImporter || useExporter) && (relevantExportCategory || relevantImportCategory) ){
			...
			//console.info(particleCount);
			
			//初始化的线上的点被过滤了一部分，或者说只显示一部分啊
			particleCount = constrain(particleCount,1,100);
			//显示全部的点
			particleCount = set.lineGeometry.vertices.length;
	...
	//console.log(linesGeo);

	linesGeo.colors = lineColors;	

	//	make a final mesh out of this composite
	//点上面的线
	//var splineOutline = new THREE.Line( new THREE.Geometry(), new THREE.LineBasicMaterial( 
	...
	// 普通材质
	//var pSystem = new THREE.ParticleSystem( particlesGeo, particleMat );
...
function selectVisualization( linearData, year, countries, exportCategories, importCategories ){
	...
	//highlightCountry( ["GERMANY", "MONGOLIA", "SWITZERLAND"] );
