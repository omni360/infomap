...
//着色器颜色
var exportColor = 0x154492;
var importColor = 0xdd380c;
...
// 			specular: 	0x333333,
    ...
    var Shaders = {
        ...
        'atmosphere' : {
            ...
            fragmentShader : ['varying vec3 vNormal;', 'void main() {', 'float intensity = pow( 0.8 - dot( vNormal, vec3( 0, 0, 1.0 ) ), 12.0 );', 
            'gl_FragColor = vec4( 1.0, 1.0, 1.0, 0.6 ) * intensity;', '}'].join('\n')
        }
    };
    ...
    var the_material = new THREE.ShaderMaterial({
        uniforms : uniforms,
        vertexShader : shader.vertexShader,
        fragmentShader : shader.fragmentShader,
    });
    var mesh = new THREE.Mesh(new THREE.SphereGeometry( 100, 40, 40 ), the_material);
    mesh.scale.x = mesh.scale.y = mesh.scale.z = 1.05;
    mesh.flipSided = true;
    ...
    // timeBins 是all.json的数据，将其初始化
    //console.info(timeBins);
	...
	// 经纬度数据 load geo data (country lat lons in this case)
	...
	// 初始选中美国
	//数据初始
    selectionData.selectedYear = '1996';
    selectionData.selectedCountry = 'China';
    ...
    // console.info(selectionData.getImportCategories());
	selectVisualization( timeBins, '1996', ['China'], [], ['Military Weapons','Civilian Weapons', 'Ammunition'] );					
...
function animate() {	
    ...
    if(start_flag){
        if(camera.position.z < 1600){
            camera.position.z = camera.position.z + 12;
        }
        else{
            start_flag = false;
        }
...
function highlightCountry( countries ){	
	...
	//console.info(countryCodes.length);
    ...
    //console.info(selectedCountry.countryCode);
